...
class AttH(BaseH):
    ...
    def __init__(self, args):
        ...
        self.rel_diag_rail = nn.Embedding(self.sizes[1], 2 * self.rank)
        self.rel_diag_rail.weight.data = 2 * torch.rand((self.sizes[1], 2 * self.rank), dtype=self.data_type) - 1.0
    ...
    # def get_rhs(self, queries, eval_mode): #ours 改变
    #     if eval_mode:
    #         return self.entity.weight, self.bt.weight
    #     else:
    #         tail = self.entity(queries[:, 2])
    #         rot_mat, _ = torch.chunk(self.rel_diag_rail(queries[:, 1]), 2, dim=1)
    #         res11 = givens_rotations(rot_mat, tail)
    #         return res11, self.bt(queries[:, 2])
